    ...
    <div x-data="{
        ...
        addToCart(product) {
            if (product.quantity > 0) {
                ...
                console.log('Буюм арабага кошулду:', cartItem);
            } else {
                console.warn('Товарлар кампада жок.');
            }
        }
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div class="hidden lg:flex flex-col space-y-4 lg:col-span-1">
                <button title="Мурунку сүрөт" class="bg-[#FFF4E9] p-2 rounded-full text-[#FF9027] transition flex items-center justify-center hover:bg-gray-200">
                ...
                <button title="Кийинки сүрөт" class="bg-[#FFF4E9] p-2 rounded-full text-[#FF9027] transition flex items-center justify-center hover:bg-gray-200">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="lg:col-span-11 flex flex-col md:flex-row gap-8">
                ...
                <div class="w-full md:w-2/5 flex flex-col space-y-6">
                    <div class="flex justify-end space-x-3">
                        <div x-data>
                            <button
                                    ...
                                    :class="$store.wishlist.isIn(product.id) ? 'text-red-500 bg-red-100' : 'text-[#FF9027] bg-[#FFF4E9]'"
                                    class="p-3 rounded-[12px] transition"
                        ...
                        <div x-data="{ productUrl: '{{ route('products.show', $product->slug) }}' }">
                            <button
                                    title="Бөлүшүү"
                                    class="p-3 rounded-[12px] bg-gray-100 text-gray-600 hover:bg-gray-200 transition"
                                    @click="if(navigator.share){
                                           navigator.share({ title: '{{ $product->name }}', url: productUrl });
                                       } else {
                                           prompt('Шилтемени көчүрүү', productUrl);
                                       }"
                            >
                    ...
                    <button @click.prevent="addToCart(product)"
                            ...
                            :class="{
                                'bg-[#FF9027] text-white': product.quantity > 0,
                                'bg-[#FF9027] text-white cursor-not-allowed opacity-70': product.quantity <= 0
                            }"
                        ...
                        <span x-text="product.quantity > 0 ? 'Себет' : 'Кампада жок'"></span>
                    </button>
